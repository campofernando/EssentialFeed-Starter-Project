...
class CodableFeedStore {
    ...
    func insert(_ feed: [LocalFeedImage], timestamp: Date, completion: @escaping FeedStore.InsertionCacheCompletion) {
...
class CodableFeedStoreTests: XCTestCase {
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    ...
                    default:
                        XCTFail(
                        """
                        Expected retrieving twice from non empty cache to deliver same found result with feed \(feed) 
                        and timestamp \(timestamp), got \(firstResult) and \(secondResult) instead
                        """
                        )
                    }
    ...
    private func makeSUT(file: StaticString = #filePath, line: UInt = #line) -> CodableFeedStore {
        let sut = CodableFeedStore(storeURL: storeSpecificURL())
    ...
    private func deleteStoreArtifacts() {
        try? FileManager.default.removeItem(at: storeSpecificURL())
    }
    
    private func storeSpecificURL() -> URL {
